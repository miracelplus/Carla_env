...
#@pysnooper.snoop("/log/get_closest_lane_index.log")
    def next_lane(self, current_index, route=None, position=None, np_random=np.random):
        ...
        #Modified by Haowei Sun 20190706
        if not next_to:
            #print("not next to")
            try:
...
class Road(Loggable):
    ...
    # Xintao get all BV within the observation range
    # Haowei modify: remove CAV in get_BV result 20191123
    def get_BV(self, vehicle, count, observation_range=100000):
        sorted_v = sorted([v for v in self.vehicles[1:]
                           if v is not vehicle and abs(vehicle.lane_distance_to(v)) < observation_range],
                          key=lambda v: abs(vehicle.lane_distance_to(v)))
        return sorted_v[:count]

    def get_BV_EU(self, vehicle, count, observation_range=100000):
        sorted_v = sorted([v for v in self.vehicles[1:]
                           if v is not vehicle and abs(vehicle.lane_distance_to(v)) < observation_range],
                          key=lambda x: np.sqrt((x.position[0] - vehicle.position[0])**2 + (x.position[1] - vehicle.position[1])**2))
        return sorted_v[:count]

    # Haowei Update 20191125 add action choose in background vehicle
    def act(self, essential_flag=False):
        """
            Decide the actions of each entity on the road.
        """
        for vehicle in self.vehicles[1:]:
            if essential_flag == False:
                if not vehicle.controlled:
                    action, ndd_possi, critical_possi = vehicle.act(None,essential_flag)
            else:
                _, _, _ = vehicle.act(None,essential_flag)                
    ...
    def step(self, dt):
        ...
        # 2020-04-26 Only check collision for the CAV
        for vehicle in self.vehicles:
    ...
    def neighbour_vehicles(self, vehicle, lane_index=None):
        ...
        # 20191008 Add Xintao. find out the neighbourhood vehicle within a observation range
    def neighbour_vehicles_within_range(self, vehicle, lane_index=None, obs_range=100):
        """
            Find the preceding and following vehicles of a given vehicle.
        :param vehicle: the vehicle whose neighbours must be found
        :param lane_index: the lane on which to look for preceding and following vehicles.
        ...
        for v in self.vehicles:
            if v is not vehicle and self.network.is_connected_road(v.lane_index, lane_index, same_lane=True):
                ...
                if not lane.on_lane(v.position, s_v, lat_v):
                    continue
                if s <= s_v and (s+obs_range) >= s_v and (s_front is None or s_v <= s_front):
                    s_front = s_v
                    v_front = v
                if s_v < s and (s_v+obs_range) >= s and (s_rear is None or s_v > s_rear):
                    s_rear = s_v
                    v_rear = v
        return v_front, v_rear
