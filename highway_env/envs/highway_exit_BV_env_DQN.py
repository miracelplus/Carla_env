...
class HighwayExitEnvNDD_DQN(HighwayExitEnvNDD):
    """
    ...
    def __init__(self, config):
        ...
        self.env_obs_num = 10
        super(HighwayExitEnvNDD_DQN, self).__init__(config)

    def reset(self):
        return super(HighwayExitEnvNDD_DQN, self).reset()
    ...
    #     # whole_bvs_observation = self.get_single_vehicle_observation(self.vehicle,True,self.env_obs_num)
    #     # whole_bvs_observation = self.Vehicle_list_to_nparray_matrix(whole_bvs_observation)
    #     whole_bvs_observation = self.get_single_vehicle_observation(self.vehicle,True,self.env_obs_num)
    #     control_item = 0
    #     CAV_plus_not_controlled_bvs = [vehicle for vehicle in whole_bvs_observation if not vehicle.controlled]
    #     whole_bvs_observation = [CAV_plus_not_controlled_bvs[0]] +  self.controlled_bvs + CAV_plus_not_controlled_bvs[1:]
    #     assert len(whole_bvs_observation) == self.env_obs_num + 1
    #     whole_bvs_observation = self.Vehicle_list_to_nparray_matrix(whole_bvs_observation)
