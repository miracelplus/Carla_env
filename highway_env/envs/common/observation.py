...
class KinematicObservation(ObservationType):
    ...
    def __init__(self, env, features=FEATURES, vehicles_count=5, **kwargs):
        ...
        self.features_acc_training = self.FEATURES_acc_training  # Xintao add
    ...
    # Xintao add 2019810 to get observation, 100 range of ego-vehicle and maximum 10 vehicles
    def observe_acc_training(self):
        # Add ego-vehicle
        ...
        _vehicle_count = 5  # maximum number of observed BV
        close_vehicles = self.env.road.get_BV(self.env.vehicle, _vehicle_count)
        if close_vehicles:
            df = df.append(pandas.DataFrame.from_records(
                [v.to_dict(self.env.vehicle)
                 for v in close_vehicles[:_vehicle_count]])[self.features],
                           ignore_index=True)
        # Normalize
        df = self.normalize_acc_training(df, self.observation_range)
        # Fill missing rows
        if df.shape[0] < _vehicle_count:
            rows = -np.ones((_vehicle_count + 1 - df.shape[0], len(self.features)))
            df = df.append(pandas.DataFrame(data=rows, columns=self.features), ignore_index=True)
        # Reorder
        df = df[self.features]
        # Clip
    ...
    # Xintao add 2019811 to get original observation
    ...
    # Xintao add 2019810 to normalize, 100m range of ego-vehicle
    def normalize_acc_training(self, df, observation_range):
        """
            Normalize the observation values.
        ...
        """
        side_lanes = self.env.road.network.all_side_lanes(self.env.vehicle.lane_index)
        x_position_range = self.observation_range
        y_position_range = AbstractLane.DEFAULT_WIDTH * len(side_lanes)
